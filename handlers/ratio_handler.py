...
def ratio_provinces_table(df: pd.DataFrame, for_pile: bool = True) -> pd.DataFrame:
    ...
    if prov_col is None:
        return pd.DataFrame(columns=["排名", "省份", "车桩比", "保有量", "充电桩"])
...
def ratio_vehicle_pile_product_table(df: pd.DataFrame) -> pd.DataFrame:
    """
    车桩比产品表（标准列）：省份、新能源车保有量、公共充电设施数量、车桩比。
    省份分组用「省份_中文」，否则「省份」。
    ...
    out = grp[["省份", "新能源车保有量", "公共充电设施数量", "车桩比"]]
    return append_taiwan_placeholder_row(out, fill_value="\\")
