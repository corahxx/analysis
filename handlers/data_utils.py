...
def province_names_with_taiwan(names: Sequence[object]) -> List[str]:
    """返回去重后的省份名列表，并确保包含台湾省；未知排最后。"""
    out: List[str] = []
    seen = set()
    for v in names or []:
        ...
        seen.add(s)
        out.append(s)
    if TAIWAN_PROVINCE not in seen:
        out.append(TAIWAN_PROVINCE)
    return sorted(out, key=lambda x: (x == "未知", x))


def append_taiwan_placeholder_row(
    df: pd.DataFrame,
    province_col: str = "省份",
    *,
    fill_value="",
    ...
    """若表中无台湾省，则追加一行占位。"""
    if df is None or province_col not in df.columns:
        return df
    cur = df.copy()
    names = cur[province_col].fillna("").astype(str).str.strip()
    if (names == TAIWAN_PROVINCE).any():
        return cur
    row = {c: fill_value for c in cur.columns}
    row[province_col] = TAIWAN_PROVINCE
    return pd.concat([cur, pd.DataFrame([row])], ignore_index=True)
