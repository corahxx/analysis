...
_POWER_SEGMENT_LABELS = ["p < 120kW", "120-250kW", "250-480kW", "480-960kW", "960kW+"]
...
def fill_workbook(path: str, prev_counts: Dict[Tuple[str, str], float]) -> int:
    ...
    filled = 0
    try:
        has_taiwan_sheet = _TAIWAN_PROVINCE in wb.sheetnames
        for ws in wb.worksheets:
            ...
            if has_prov and pi is not None:
                ...
                for r in range(2, ws.max_row + 1):
                    ...
                    if str(pv).strip() == _TAIWAN_PROVINCE:
                        has_tw = True
                        break
                if not has_tw:
                    for seg in _POWER_SEGMENT_LABELS:
                        ws.append([_TAIWAN_PROVINCE, seg, "\\", "\\", "\\"])
            ...
            for r in range(2, ws.max_row + 1):
                ...
                if prov == _TAIWAN_PROVINCE and curr is None:
                    ws.cell(row=r, column=mi + 1).value = "\\"
                    filled += 1
                    continue
                ws.cell(row=r, column=mi + 1).value = _fmt_mom_growth(curr, prev_v)
                filled += 1
        if not has_taiwan_sheet:
            ws = wb.create_sheet(_TAIWAN_PROVINCE)
            ws.append(["功率段", "数量", "占比", "环比"])
            for seg in _POWER_SEGMENT_LABELS:
                ws.append([seg, "\\", "\\", "\\"])
        wb.save(path)
    finally:
